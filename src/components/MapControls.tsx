...
const MESSAGES = defineMessages({
  en: {
    ...
    panEast: 'Pan east',
    panSouth: 'Pan south',
    holdHint: '{label} — hold to keep going',
  },
  it: {
    group: 'Controlli camera mappa',
    zoomIn: 'Ingrandisci',
    zoomOut: 'Riduci',
    panNorth: 'Sposta a nord',
    panWest: 'Sposta a ovest',
    panEast: 'Sposta a est',
    panSouth: 'Sposta a sud',
    holdHint: '{label} — tieni premuto per continuare',
  },
});
    ...
    <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }}
      ...
      <div role="group" aria-label={t('group')}
        ...
        <div className="flex flex-col gap-[3px]">
          <Btn t={t} label={t('zoomIn')} move={{ kind: 'zoom', dir: 1 }} icon={Plus} disabled={limits.max} press={press} release={release} step={step} />
          <Btn t={t} label={t('zoomOut')} move={{ kind: 'zoom', dir: -1 }} icon={Minus} disabled={limits.min} press={press} release={release} step={step} />
        </div>
        <div className="desktop-only flex items-center gap-1">
          ...
          <div className="grid grid-cols-3 grid-rows-3 gap-[3px]">
            <Btn t={t} cell="col-start-2 row-start-1" label={t('panNorth')} move={{ kind: 'pan', dx: 0, dy: -1 }} icon={ChevronUp} press={press} release={release} step={step} />
            <Btn t={t} cell="col-start-1 row-start-2" label={t('panWest')} move={{ kind: 'pan', dx: -1, dy: 0 }} icon={ChevronLeft} press={press} release={release} step={step} />
            <Btn t={t} cell="col-start-3 row-start-2" label={t('panEast')} move={{ kind: 'pan', dx: 1, dy: 0 }} icon={ChevronRight} press={press} release={release} step={step} />
            <Btn t={t} cell="col-start-2 row-start-3" label={t('panSouth')} move={{ kind: 'pan', dx: 0, dy: 1 }} icon={ChevronDown} press={press} release={release} step={step} />
          </div>
        </div>
      </div>
    </motion.div>
...
function Btn({ t, cell = '', label, icon: Icon, move, disabled = false, press, release, step }: {
  t: (key: 'holdHint', vars?: Record<string, string | number>) => string;
  ...
  return (
    <button type="button" title={t('holdHint', { label })} aria-label={label} disabled={disabled}
