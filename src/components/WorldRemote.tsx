...
const MESSAGES = defineMessages({
  en: {
    ...
    tabLog: 'LOG',
    hitScanPre: 'Hit',
    hitScanPost: 'to acquire targets',
    emptyTags: 'Auto-vacuum · GPS-tagged · Vault',
    btUnavailable: 'Web Bluetooth unavailable — use Chrome/Edge on localhost',
    live: 'LIVE',
    vacuuming: 'VACUUMING',
    svcsMs: '{svcs}svcs · {ms}ms',
    deviceIntel: 'DEVICE INTEL',
    ...
    txPower: 'TX Power',
    extracted: 'Extracted',
    extractedStats: '{svcs}svcs · {chars}chars · {vals}vals · {bytes}B',
    vacuumDump: 'VACUUM DUMP ({n})',
    drop: 'DROP',
    connect: 'CONNECT',
    pairing: 'PAIRING...',
    gattTree: 'GATT TREE',
    ...
    live2: 'LIVE',
    sub: 'SUB',
    networkRecon: 'NETWORK RECON',
    probe: 'PROBE',
    probeShort: '...',
    tapProbe: 'Tap PROBE to scan local network',
    localIps: 'LOCAL IPs',
    down: 'DOWN',
    openPorts: 'OPEN PORTS',
    vaultExport: 'VAULT & EXPORT',
    deviceCount: '{n} device · {size}',
    deviceCountMany: '{n} devices · {size}',
    wipe: 'WIPE',
    pktsSize: '{n} pkts · {size}',
    pause: 'PAUSE',
    liveLog: 'LIVE',
    clr: 'CLR',
    scanToSee: 'Scan to see packets...',
    standby: 'STANDBY',
    liveN: '{n} LIVE',
  },
  it: {
    ...
    scan: 'CERCA',
    scanShort: '...',
    viewOnMap: 'MOSTRA SU MAPPA',
    fullScreen: 'Schermo intero',
    tabDevices: 'DISPOSITIVI',
    tabIntel: 'INTEL',
    tabLog: 'LOG',
    hitScanPre: 'Premi',
    hitScanPost: 'per acquisire bersagli',
    emptyTags: 'Auto-vacuum · GPS · Vault',
    btUnavailable: 'Web Bluetooth non disponibile — usa Chrome/Edge su localhost',
    live: 'LIVE',
    vacuuming: 'ASPIRAZIONE',
    svcsMs: '{svcs}svc · {ms}ms',
    deviceIntel: 'INTEL DISPOSITIVO',
    ...
    txPower: 'Potenza TX',
    extracted: 'Estratti',
    extractedStats: '{svcs}svc · {chars}car · {vals}val · {bytes}B',
    vacuumDump: 'DUMP ASPIRAZIONE ({n})',
    drop: 'STACCA',
    connect: 'CONNETTI',
    pairing: 'PAIRING...',
    gattTree: 'ALBERO GATT',
    enumerating: 'Enumerazione...',
    read: 'LEGGI',
    live2: 'LIVE',
    sub: 'SUB',
    networkRecon: 'RICOGNIZIONE RETE',
    probe: 'SONDA',
    probeShort: '...',
    tapProbe: 'Tocca SONDA per scansionare la rete locale',
    localIps: 'IP LOCALI',
    down: 'DOWN',
    openPorts: 'PORTE APERTE',
    vaultExport: 'VAULT & ESPORTA',
    deviceCount: '{n} dispositivo · {size}',
    deviceCountMany: '{n} dispositivi · {size}',
    wipe: 'CANCELLA',
    pktsSize: '{n} pkt · {size}',
    pause: 'PAUSA',
    liveLog: 'LIVE',
    clr: 'CLR',
    scanToSee: 'Cerca per vedere i pacchetti...',
    standby: 'STANDBY',
    liveN: '{n} LIVE',
  },
});
    ...
    <div className={`glass-panel flex flex-col overflow-hidden pointer-events-auto shrink-0 ${full?'w-full h-full':'h-[500px] max-h-[80vh] resize-y'}`}>
      ...
      <div className="flex-shrink-0 flex items-center justify-between px-3 py-2.5 border-b border-[rgba(255,255,255,0.05)] bg-[rgba(0,0,0,0.3)] hover:bg-[var(--hover-accent)] transition-colors">
        <button onClick={()=>setExpanded(!expanded)} className="flex items-center gap-2 flex-1">
          ...
          <span className="gotham-tag gotham-tag--info" style={{fontSize:'9px',padding:'1px 5px'}}>{t('devs',{n:devices.length})}</span>
          {scanning&&<span className="gotham-tag" style={{fontSize:'9px',padding:'1px 5px',background:'rgba(0,230,255,0.1)',color:'var(--cyan-primary)'}}>{t('scanning')}</span>}
        </button>
        <div className="flex items-center gap-2">
          <motion.button whileTap={{scale:0.95}} onClick={scan} disabled={scanning||!btOk}
            ...
            {scanning?<BluetoothSearching className="w-3 h-3 animate-pulse"/>:<Bluetooth className="w-3 h-3"/>}{scanning?t('scanShort'):t('scan')}
          </motion.button>
          ...
          <button onClick={()=>setFull(!full)} className="p-1.5 -m-0.5 rounded text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-white/10 transition-colors" title={t('fullScreen')}>
      ...
      <AnimatePresence>{expanded&&(
        <motion.div initial={{height:0,opacity:0}} animate={{height:'auto',opacity:1}} exit={{height:0,opacity:0}} transition={{duration:0.2}} className="flex-1 flex flex-col overflow-hidden min-h-0">
          ...
          <div className="flex items-center px-2 py-1.5 shrink-0 gap-0.5 border-b border-[rgba(255,255,255,0.04)]" style={{background:'rgba(0,0,0,0.2)'}}>
            {([['scan','tabDevices',Scan],['intel','tabIntel',Globe],['log','tabLog',Terminal]] as const).map(([id,label,Icon])=>(
                ...
                {devices.length===0&&!scanning&&btOk&&(
                  <div className="flex flex-col items-center justify-center py-8 opacity-40">
                    ...
                    <p className="text-[10px] font-mono text-[var(--text-muted)] text-center tracking-wider">
                      {t('hitScanPre')} <span className="text-[var(--cyan-primary)]">{t('scan')}</span> {t('hitScanPost')}<br/>
                      <span className="text-[9px] opacity-50">{t('emptyTags')}</span>
                    </p>
                  </div>
                )}
                {!btOk&&<div className="p-2 rounded-lg" style={{background:'rgba(255,61,61,0.04)',border:'1px solid rgba(255,61,61,0.08)'}}><span className="text-[9px] font-mono text-[#FF3D3D]">{t('btUnavailable')}</span></div>}
                    ...
                    <div className="rounded-xl overflow-hidden transition-all" style={{background:dev.connected?`linear-gradient(135deg,${dev.color}06,${dev.color}02)`:'rgba(255,255,255,0.015)',border:`1px solid ${dev.connected?dev.color+'15':'rgba(255,255,255,0.04)'}`,boxShadow:dev.connected?`0 0 16px ${dev.color}08`:'none'}}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {dev.probing?(
                            ...
                            <div className="flex items-center gap-1.5 flex-wrap">
                              ...
                              <span className="text-[9px] font-mono text-[var(--text-muted)]">{t('svcsMs',{svcs:dev.serviceCount,ms:dev.probeMs??0})}</span>
                            </div>
                          )}
                        </div>
                      ...
                      <AnimatePresence>{isE&&!dev.probing&&(<motion.div initial={{height:0,opacity:0}} animate={{height:'auto',opacity:1}} exit={{height:0,opacity:0}} className="overflow-hidden"><div className="px-3 pb-3" style={{borderTop:'1px solid rgba(255,255,255,0.03)'}}>
                        ...
                        <div className="mt-2 mb-2.5 rounded-lg p-2.5" style={{background:'rgba(0,0,0,0.25)',border:'1px solid rgba(255,255,255,0.03)'}}>
                          <div className="text-[9px] font-mono text-[var(--text-muted)] tracking-[0.2em] mb-1.5 font-bold">{t('deviceIntel')}</div>
                          <div className="space-y-1">
                            {[{k:t('manufacturer'),v:dev.manufacturer},{k:t('model'),v:dev.model},{k:t('serial'),v:dev.serial},{k:t('firmware'),v:dev.firmware},{k:t('txPower'),v:dev.txPower!=null?`${dev.txPower} dBm`:undefined}].filter(x=>x.v).map(x=>(
                            ...
                            <div className="flex gap-2"><span className="text-[9px] font-mono text-[var(--text-muted)] w-[65px] shrink-0 tracking-wider">{t('extracted')}</span><span className="text-[9px] font-mono text-[var(--text-muted)]">{t('extractedStats',{svcs:dev.serviceCount,chars:dev.charCount,vals:dev.gattDump.length,bytes:dev.totalBytes})}</span></div>
                          </div>
                        </div>
                        ...
                        {/* GATT Dump */}
                        {dev.gattDump.length>0&&<div className="mb-2.5"><div className="text-[9px] font-mono text-[var(--text-muted)] tracking-[0.15em] mb-1 font-bold">{t('vacuumDump',{n:dev.gattDump.length})}</div><div className="max-h-[120px] overflow-y-auto styled-scrollbar rounded-lg" style={{background:'rgba(0,0,0,0.3)'}}>
                        ...
                        <AnimatePresence>{isG&&(<motion.div initial={{height:0,opacity:0}} animate={{height:'auto',opacity:1}} exit={{height:0,opacity:0}} className="overflow-hidden"><div className="mt-2.5 pt-2.5" style={{borderTop:`1px solid ${dev.color}08`}}>
                          <div className="flex items-center justify-between mb-2"><span className="text-[9px] font-mono font-bold tracking-[0.12em]" style={{color:dev.color}}>{t('gattTree')}</span><button onClick={()=>{setGattTarget(null);setGattSvcs([]);}}><X className="w-2.5 h-2.5 text-[var(--text-muted)]"/></button></div>
                          {gattLoading?<div className="flex items-center justify-center py-3 gap-1.5"><div className="w-3.5 h-3.5 border-2 rounded-full animate-spin" style={{borderColor:dev.color,borderTopColor:'transparent'}}/><span className="text-[9px] font-mono animate-pulse" style={{color:dev.color}}>{t('enumerating')}</span></div>
              ...
              <div className="flex-1 overflow-y-auto styled-scrollbar p-2.5 space-y-2.5">
                ...
                <div className="rounded-xl overflow-hidden" style={{background:'rgba(255,255,255,0.015)',border:'1px solid rgba(255,255,255,0.04)'}}>
                  <div className="px-3 py-2 flex items-center justify-between" style={{borderBottom:'1px solid rgba(255,255,255,0.03)'}}>
                    <div className="flex items-center gap-1.5"><Globe className="w-3 h-3 text-[#80DEEA]"/><span className="text-[9px] font-mono font-bold tracking-[0.12em] text-[var(--text-primary)]">{t('networkRecon')}</span></div>
                    <motion.button whileTap={{scale:0.95}} onClick={runNetRecon} disabled={netLoading} className="px-2 py-0.5 rounded text-[9px] font-mono font-bold tracking-wider disabled:opacity-30" style={{background:'rgba(128,222,234,0.06)',color:'#80DEEA',border:'1px solid rgba(128,222,234,0.08)'}}>{netLoading?t('probeShort'):t('probe')}</motion.button>
                  </div>
                  {!netIntel&&<div className="px-3 py-4 text-center"><span className="text-[9px] font-mono text-[var(--text-muted)] opacity-30">{t('tapProbe')}</span></div>}
                  {netIntel&&<div className="p-2.5 space-y-2">
                    {netIntel.localIPs.length>0&&<div><div className="text-[9px] font-mono text-[var(--text-muted)] tracking-wider mb-1 font-bold">{t('localIps')}</div>{netIntel.localIPs.map((ip,i)=><div key={i} className="text-[10px] font-mono text-[#80DEEA] pl-2">{ip}</div>)}</div>}
                    {netIntel.type&&<div className="flex gap-3">{[{l:'TYPE',v:netIntel.type},{l:t('down'),v:netIntel.downlink!=null?`${netIntel.downlink}Mbps`:undefined},{l:'RTT',v:netIntel.rtt!=null?`${netIntel.rtt}ms`:undefined}].filter(x=>x.v).map(x=><div key={x.l}><div className="text-[9px] font-mono text-[var(--text-muted)] tracking-wider">{x.l}</div><div className="text-[10px] font-mono text-[var(--text-primary)] font-bold">{x.v}</div></div>)}</div>}
                    {netIntel.openPorts.length>0&&<div><div className="text-[9px] font-mono text-[var(--text-muted)] tracking-wider mb-1 font-bold">{t('openPorts')}</div>{netIntel.openPorts.map((p,i)=><div key={i} className="flex items-center gap-2 pl-2 py-0.5"><span className="text-[9px] font-mono font-bold text-[var(--alert-green)]">:{p.port}</span><span className="text-[9px] font-mono text-[var(--text-muted)]">{p.service}</span></div>)}</div>}
                  </div>}
                </div>
                {/* Export */}
                <div className="rounded-xl overflow-hidden" style={{background:'rgba(255,255,255,0.015)',border:'1px solid rgba(255,255,255,0.04)'}}>
                  <div className="px-3 py-2 flex items-center gap-1.5" style={{borderBottom:'1px solid rgba(255,255,255,0.03)'}}><Database className="w-3 h-3 text-[#FFB800]"/><span className="text-[9px] font-mono font-bold tracking-[0.12em] text-[var(--text-primary)]">{t('vaultExport')}</span></div>
                  <div className="p-2.5 space-y-1.5">
                    <div className="text-[9px] font-mono text-[var(--text-muted)]">{t(vaultCount!==1?'deviceCountMany':'deviceCount',{n:vaultCount,size:bytes>1024?`${(bytes/1024).toFixed(1)}KB`:`${bytes}B`})}</div>
            ...
            {view==='log'&&(
              <div className="flex-1 flex flex-col overflow-hidden">
                <div className="flex items-center justify-between px-3 py-1 shrink-0" style={{background:'rgba(0,0,0,0.3)',borderBottom:'1px solid rgba(255,255,255,0.03)'}}>
                  <span className="text-[9px] font-mono text-[var(--text-muted)] tracking-wider">{t('pktsSize',{n:pkts.length,size:bytes>1024?`${(bytes/1024).toFixed(1)}KB`:`${bytes}B`})}</span>
                  <div className="flex items-center gap-1.5">
                    <button onClick={()=>setPaused(!paused)} className={`text-[9px] font-mono tracking-wider px-1 py-0.5 rounded flex items-center gap-0.5 ${paused?'text-[#FFB800]':'text-[var(--text-muted)]'}`}>{paused?<><Pause className="w-2 h-2"/>{t('pause')}</>:<><Play className="w-2 h-2"/>{t('liveLog')}</>}</button>
                    <button onClick={()=>{setPkts([]);setBytes(0);pid.current=0;}} className="text-[9px] font-mono text-[var(--text-muted)] tracking-wider px-1 py-0.5 rounded hover:bg-white/5 flex items-center gap-0.5"><Trash2 className="w-2 h-2"/>{t('clr')}</button>
                  </div>
                </div>
                <div ref={logEl} className="flex-1 overflow-y-auto styled-scrollbar font-mono" style={{background:'#020406'}}>
                  {pkts.length===0?<div className="flex items-center justify-center h-full opacity-15"><span className="text-[9px] font-mono text-[var(--text-muted)] tracking-wider">{t('scanToSee')}</span></div>
          ...
          <div className="flex items-center justify-between px-3 py-1.5 shrink-0" style={{borderTop:'1px solid rgba(255,255,255,0.03)',background:'rgba(0,0,0,0.2)'}}>
            <div className="flex items-center gap-1.5"><div className={`w-1.5 h-1.5 rounded-full ${btOk?'bg-[var(--alert-green)]':'bg-[#FF3D3D]'} ${scanning?'animate-pulse':''}`}/><span className="text-[9px] font-mono text-[var(--text-muted)] tracking-wider">{scanning?t('scanning'):liveCount>0?t('liveN',{n:liveCount}):t('standby')} · {upStr}</span></div>
