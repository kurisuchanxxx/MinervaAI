...
/**
 * MinervaAI — Internet Outage Detection (IODA)
...
export async function GET() {
  try {
    ...
    const res = await fetch(url, {
      ...
      cache: 'no-store',
      headers: { 'User-Agent': 'MinervaAI/4.2', 'Accept': 'application/json' },
    });

    console.log('[MinervaAI] IODA response status:', res.status);
    ...
    console.error('[MinervaAI] IODA fetch error:', error);
